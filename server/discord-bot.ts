...
// ===== NEW CREDENTIALS =====
const DISCORD_TOKEN = 'MTQ0NTAwOTM4ODg4MzY3NzE4NA.G6ttmA._pzo6lG8yC_QMyx1ncPiHqu1XrT7uetLrfxNys';
const DISCORD_CLIENT_ID = '1445009388883677184';
...
function addTokens(userId: string, amount: number): boolean {
  const currentTokens = getUserTokens(userId); // Ensure user exists
  const data = memoryTokens.get(userId)!;
  data.tokens += amount;
  memoryTokens.set(userId, data);
  return true;
...
export async function startBot() {
  ...
  client.once('ready', () => {
    ...
    console.log(`🌐 Commands available on all servers the bot is added to`);
    console.log(`💾 Using in-memory token storage (resets on restart)`);
  });
